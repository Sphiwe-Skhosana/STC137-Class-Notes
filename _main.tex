...
\item
  ...
  \textbf{Eating ice cream causes drowning}. This is an example of confusing correlation with causation.
\item
  Correct interpretation

  There is no causal relationship between ice cream sales and drowning incidents. Instead both variables are influenced by a third variable \textbf{hot weather}. Hot weather increases the likelihood of people swimming in pools, lakes, or the ocean, which in turn increases the risk of drowning. At the very same time, hot weather also increases the demand for ice cream, as people seek cold treats to cool down.

  Thus, the correlation between ice cream sales and drowning incidents is \textbf{spurious}. Spurious correlation arises when two variables are coincidentally related or influenced by the same third variable, known as a confounding variable, such that they appear to be causally related.
\end{itemize}
...
\caption{A box-and-whisker plot showing an outlier}\label{fig:unnamed-chunk-6}
\end{figure}
...
\caption{Scatter plot of electric vehicle sales (in R millions) and the number of advertisements}\label{fig:unnamed-chunk-7}
\end{figure}
